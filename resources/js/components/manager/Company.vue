...
<v-row class="justify-content-center bg-light">
  <v-col cols="12" class="justify-content-center">
         ...
         <v-data-table
          ...
          >
            ...
            <template v-slot:item.user.isActive="{ item }">
                <v-chip color="teal" text-color="white"  small v-if="item.user.isActive == 1" >نشــط</v-chip>
                <v-chip class="ma-2" color="pink" text-color="white"  small v-else >غير نشــط</v-chip>
            </template>
          </v-data-table>
      </v-card-text>
    </v-card>
    <v-dialog
      ...
      max-width="650">
      <v-card class="col-12">
        <v-card-title  class="headline text-primary">أضف جهة منفذة</v-card-title>
        <v-card-text>
      ...
      <v-card class="col-12">
        ...
        <v-card-text>
          <v-form ref='city' v-model="valid">
            <v-row>
              <v-col>
                <v-text-field v-if="selectedUser.company" readonly v-model="selectedUser.company.name" placeholder="  إسم الجهة " prepend-icon="mdi-home-variant" :rules="nameRules" color="#0c8676"></v-text-field>
                <v-text-field v-if="selectedUser.company" v-model="selectedUser.company.regulator" placeholder="  الجهة التابع لها  " prepend-icon="mdi-home-city" color="#0c8676" :rules="nameRules"></v-text-field>
                <v-text-field v-if="selectedUser.company" v-model="selectedUser.company.clearance_no" placeholder="  رقم التصريح" prepend-icon="mdi-qrcode-scan" color="#0c8676" :rules="nameRules"></v-text-field>
                <!-- <v-text-field v-if="selectedUser.company" v-model="selectedUser.company.city" placeholder="  العنــوان " prepend-icon="mdi-marker" color="#0c8676" :rules="nameRules"></v-text-field> -->
              </v-col>
              <v-col>
    ...
    export default {
      ...
      data () {
        return {
          ...
          users:[],
          usersheaders:[
            {text: 'إسم الجهة', value: 'name'},
            {text: 'الجهة التابع لها ', value: 'regulator'},
            {text: 'المنطقة', value: 'city.name'},
            {text: 'الجوال', value: 'phone'},
            {text: 'الإيميل', value: 'user.email'},
            {text: 'حالة الحساب', align: 'center', value: 'user.isActive'},
      ...
      methods:{
        ...
        getUsers(){
          let self = this;
            axios
          .get(`/api/managercompany?manager=${self.user}`)
