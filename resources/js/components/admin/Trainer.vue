         ...
         <v-data-table
          ...
          >
            ...
            <template v-slot:item.created_at="{ item }">
              <v-img v-if="item.trainer"
              class="elevation-2"
              max-height="50"
              max-width="60"
              :src=" '../storage/'+item.trainer.path "
              @click="showPic(item.trainer)"
             ></v-img>
            </template>
            <!-- <template v-slot:item.created_at="{ item }">{{item.created_at | momentDate}}</template> -->
            <template v-slot:item.updated_at="{ item }">
              <v-icon @click="editUser(item)">mdi-pen</v-icon>
              <v-icon v-if="item.trainer && !item.trainer.rate" color="teal" @click="assesTrainer(item)">mdi-star</v-icon>
              <v-icon v-else color="teal" @click="getAssessment(item.trainer)">mdi-account-star-outline</v-icon>
              </template>
              <template v-slot:item.id="{ item }">
              <!-- <v-icon>mdi-paperclip</v-icon> -->
              <a v-if="item.trainer && item.trainer.cert_path " rel="icon" target="_blank" :href="'../storage/'+item.trainer.cert_path"> <v-icon>mdi-paperclip</v-icon></a>
              </template>
    ...
    <v-dialog
      ...
      <v-card class="col-12">
        <v-card-text>
          ...
          <v-text-field color="teal" v-model="asses_desc" placeholder="نص التقـــييم"></v-text-field>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="teal" text small class="white--text" @click="assesdialog=false">إلغاء</v-btn>
          <v-btn color="teal" small class="white--text" @click="submitAssessment()">حفــظ</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
           <v-dialog
...
<script>
    ...
    export default {
      ...
      methods:{
        ...
        submitAssessment(){
          axios
          .put(`/api/assesment`,{trainer_id:this.assesd_trainer.trainer.id, rate:this.asses_rate,desc:this.asses_desc})
          .then((res) => {
            this.getUsers();
            this.snackbar=true;
        ...
        // },
         editUser(user){
          ...
          this.selectedcity = user.trainer.city;
